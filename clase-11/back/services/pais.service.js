...
export async function savePaisFavorito(id, usuario) {
    // _id
    //  nombre
    //  otrodato
    // 1. Iniciamos una session
    const session = client.startSession()
    try {
        // 2. Comenzar la transaccion
        session.startTransaction()

        await db.collection("ciudades").updateOne(
            { _id: new ObjectId(id) },
            { $addToSet: { favoritos: usuario } },
            { session }
        )
        const ciudad = await getPaisesById(id)
        await db.collection("usuarios").updateOne(
            { _id: new ObjectId(usuario._id) },
            { $addToSet: { favoritos: { ...ciudad, favoritos: undefined } } },
            { session }
        )
        await session.commitTransaction()

        return await getPaisesById(id)
    } catch (error) {
...
export async function deletePaisFavorito(id, usuario) {
    // 1. Iniciamos una session
    const session = client.startSession()
    try {
        // 2. Comenzar la transaccion
        session.startTransaction()
        // 1. Quitamos el usuario de la lista de favoritos de ciudad
        await db.collection("ciudades").updateOne(
            { _id: new ObjectId(id) },
            { $pull: { favoritos: usuario } },
            { session }
        )
        // 2. Quitamos la ciudad de la lista de favoritos del usuario
        await db.collection("usuarios").updateOne(
            { _id: new ObjectId(usuario._id) },
            { $pull: { favoritos: { _id: new ObjectId(id) } } },
            { session }
        )
        await session.commitTransaction()
        return await getPaisesById(id)
