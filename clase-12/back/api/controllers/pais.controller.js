...
export async function getPais(req, res) {
    try {
        const paises = await paisService.getPaises(req.query)
...
export async function savePaisFavorito(req, res) {
    try {
        const id = req.params.id
        const pais = await paisService.savePaisFavorito(id, req.body)
        res.status(201).json(pais)
    } catch (error) {
        res.status(500).json({ message: "Error al reemplazar el pais" })
    }
...
export async function deletePaisFavorito(req, res) {
    try {
        const id = req.params.id
        const pais = await paisService.deletePaisFavorito(id, req.body)
        res.status(202).json(pais)
    } catch (error) {
